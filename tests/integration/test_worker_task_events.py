...
async def test_deleted_alarm_worker_handlers_skip_external_work(
    ...
    enrich = AsyncMock()
    deliver_state = AsyncMock()
    monkeypatch.setattr("escalane.worker.tasks.enrich_alarm_context", enrich)
    monkeypatch.setattr("escalane.worker.tasks.deliver_state_webhook", deliver_state)
    ...
    deliver_state.assert_not_awaited()
...
async def test_process_alarm_event_dispatches_state_changed(
    ...
    delivered = AsyncMock()
    monkeypatch.setattr("escalane.worker.tasks.deliver_state_webhook", delivered)
