...
from escalane.notifications.targets import get_escalation_schedule, get_escalation_targets
...
async def test_escalation_schedule_rejects_legacy_conflicting_delays(sessionmaker) -> None:
    async with sessionmaker() as session:
        ...
        with pytest.raises(ConfigurationError, match="conflicting delays"):
            await get_escalation_schedule(session, "legacy-conflict")
...
async def test_build_zammad_ticket_payload():
    ...
    zammad_payload = build_zammad_ticket_payload(notification_payload, svc._zammad.config)
...
async def test_get_escalation_targets(sessionmaker, seeded_db):
    async with sessionmaker() as session:
        ...
        targets = await get_escalation_targets(session, "default", 0)
