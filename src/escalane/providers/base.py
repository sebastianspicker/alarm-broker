...
class ZammadTicketConfig(Protocol):
    """Configuration fields required to build a Zammad ticket."""

    @property
    def group(self) -> str:
        """Return the default ticket group."""

    @property
    def state_id_new(self) -> int:
        """Return the provider state for newly created tickets."""

    @property
    def customer(self) -> str:
        """Return the default ticket customer identifier."""


class ZammadTicketProvider(Protocol):
    """The narrow Zammad contract used by notification dispatch."""

    @property
    def config(self) -> ZammadTicketConfig:
        """Return ticket-creation configuration."""

    def enabled(self) -> bool:
        """Return whether ticket delivery is available."""

    async def create_ticket(self, payload: dict[str, Any]) -> int:
        """Create one ticket and return its provider ID."""

    async def add_internal_note(self, ticket_id: int, subject: str, body: str) -> None:
        """Add one private ticket note."""


class SmsProvider(Protocol):
    """The narrow SMS contract used by notification dispatch."""

    def enabled(self) -> bool:
        """Return whether SMS delivery is available."""

    async def send_sms(self, to: str, message: str) -> None:
        """Send one SMS message."""


class SignalGroupProvider(Protocol):
    """The narrow Signal contract used by notification dispatch."""

    def enabled(self) -> bool:
        """Return whether Signal delivery is available."""
