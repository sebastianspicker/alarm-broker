...
from alarm_broker.db.models import AdminAuditEvent, Device, EscalationPolicy, Person, Room, Site
from alarm_broker.services.admin_audit import (
    REDACTED_VALUE,
...
def test_deactivate_resource_checks_version_dependencies_and_bumps_version():
    resource = _Resource()

    with pytest.raises(ConflictError, match="changed since"):
        require_current_version(resource, 2)

    with pytest.raises(ConflictError, match="dependencies remain") as exc_info:
        deactivate_resource(resource, expected_version=3, dependency_counts={"rooms": 2})
    assert exc_info.value.details["dependencies"] == {"rooms": 2}

    assert deactivate_resource(resource, expected_version=3, dependency_counts={"rooms": 0}) is True
    assert resource.active is False
    assert resource.version == 4
    assert deactivate_resource(resource, expected_version=4) is False
