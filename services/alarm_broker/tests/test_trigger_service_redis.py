...
def _service(redis: Redis, *, bucket: int) -> TriggerService:
...
async def test_real_redis_uuid_bytes_cross_utf8_boundary(real_redis):
    service = _service(real_redis, bucket=uuid.uuid4().int)
...
async def test_real_redis_lock_owner_cannot_delete_replacement(real_redis):
    service = _service(real_redis, bucket=uuid.uuid4().int)
    alarm_id = uuid.uuid4()
    key = service._get_event_recovery_lock_key(alarm_id)
    try:
        await real_redis.set(key, "token-b")

        await service._release_event_recovery_lock(alarm_id, "token-a")

        expect(await real_redis.get(key) == b"token-b")
    finally:
        await real_redis.delete(key)
