"""Alarm query filters, ordering, export, and pagination edge cases."""
...
from tests.api_test_helpers import make_alarm as _make_alarm
...
async def _seed_alarm(sessionmaker, **overrides) -> None:
    """Persist one default alarm for export and list scenarios."""
    async with sessionmaker() as session:
        session.add(_make_alarm(**overrides))
...
async def test_export_json(engine, sessionmaker, seeded_db, fake_redis, settings):
    ...
    await _seed_alarm(sessionmaker)

    async with app_client(settings=settings, engine=engine, redis=fake_redis) as client:
        resp = await client.get("/v1/alarms/export", params={"format": "json"}, headers=HEADERS)
...
async def test_list_alarms_sort_asc(engine, sessionmaker, seeded_db, fake_redis, settings):
    ...
    async with app_client(settings=settings, engine=engine, redis=fake_redis) as client:
        resp = await client.get("/v1/alarms", params={"sort_order": "asc"}, headers=HEADERS)
...
async def test_cursor_pagination_asc(engine, sessionmaker, seeded_db, fake_redis, settings):
    ...
    async with app_client(settings=settings, engine=engine, redis=fake_redis) as client:
        resp = await client.get(
            "/v1/alarms", params={"limit": 2, "sort_order": "asc"}, headers=HEADERS
        )
        expect(resp.status_code == 200)
