"""Operator notes and explicitly enabled simulation endpoint tests."""
...
try:
    ...
    from tests.worker_task_helpers import open_app_client
except ModuleNotFoundError:
    from helpers import admin_login
    from helpers import trigger_alarm as _trigger_alarm
    from worker_task_helpers import open_app_client
...
async def test_simulation_endpoints_work_when_enabled(engine, seeded_db, fake_redis, settings):
    ...
    async with open_app_client(settings=settings, engine=engine, redis=fake_redis) as client:
        ...
        expect(seed_payload["seed_file"] == "simulation_seed.yaml")
