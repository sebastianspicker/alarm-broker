"""Deterministic non-secret values used by test requests and settings."""
...
def _token(label: str, value: str) -> str:
    """Create a readable, deterministic token with a test-only label."""
    return f"{label}-{value}"


def value_for_test(label: str) -> str:
    """Derive a stable placeholder value accepted by application validators."""
    return _token(label, uuid.uuid5(uuid.NAMESPACE_DNS, f"escalane-test-{label}").hex)
