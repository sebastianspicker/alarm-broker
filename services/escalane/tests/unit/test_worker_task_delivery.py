...
from escalane.worker.task_workflows import ack_url_for_alarm
...
try:
    ...
    from tests.worker_task_helpers import make_alarm, make_ctx
except ModuleNotFoundError:
    from assertions import expect
    from constants import TEST_ADMIN_API_KEY, value_for_test
    from worker_task_helpers import make_alarm, make_ctx
...
def _ticket_failure_notification() -> MagicMock:
    ...
    notification.get_escalation_schedule = AsyncMock(return_value=[])
...
def _patch_delivery_dependencies(monkeypatch, notification, enrichment):
    ...
    monkeypatch.setattr(
        "escalane.worker.tasks.successful_notification", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
...
async def test_alarm_created_schedules_escalations_before_external_side_effects(monkeypatch):
    ...
    notification = MagicMock()
    notification.get_escalation_schedule = AsyncMock(return_value=[(1, 60)])
...
async def test_alarm_created_retries_notification_audit_lookup_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        "escalane.worker.tasks.successful_notification",
...
async def test_alarm_created_raises_terminal_delivery_error_at_retry_limit(monkeypatch):
    ...
    notification = _ticket_failure_notification()
    _patch_delivery_dependencies(monkeypatch, notification, {})
    events: list[str] = []
