"""Tests for escalane.db.engine: connection pool params and slow-query listener."""
...
from escalane.db.engine import _install_slow_query_listener, create_async_engine_from_url
...
class TestCreateAsyncEngineFromUrl:
    def test_slow_query_listener_skipped_when_zero(self):
        ...
        with (
            patch("escalane.db.engine.create_async_engine") as mock_create,
            patch("escalane.db.engine._install_slow_query_listener") as mock_install,
        ):
    ...
    def test_pool_params_forwarded_to_create_async_engine(self):
        ...
        with patch("escalane.db.engine.create_async_engine") as mock_create:
            mock_engine = MagicMock()
            mock_engine.sync_engine = MagicMock()
            mock_create.return_value = mock_engine
