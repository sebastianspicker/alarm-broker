...
from escalane.connectors.mock import MockZammadClient
from escalane.core.url_validation import RetryableSSRFError, SSRFError
from escalane.db.models import Alarm, AlarmEventOutbox, AlarmNotification
...
async def test_alarm_state_changed_ssrf_blocked_logs_error(sessionmaker, seeded_db, settings):
    ...
    with patch(
        "escalane.worker.tasks.validate_url_not_internal",
...
async def test_alarm_state_changed_retries_dns_resolution_failures(
    ...
    expect(audit.error == "resolver unavailable")
...
async def test_alarm_state_changed_alarm_not_found_returns_early(sessionmaker, seeded_db, settings):
    ...
    with patch("escalane.worker.tasks.validate_url_not_internal", allow_webhook):
