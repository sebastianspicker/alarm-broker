...
from escalane.services.enrichment_service import enrich_alarm_context
...
def _retry_delivery_errors(operation: str) -> Callable[[WorkerTask], WorkerTask]:
    ...
    def decorate(task: WorkerTask) -> WorkerTask:
        ...
        @wraps(task)
        async def wrapped(ctx: dict, alarm_id: str, *args: Any, **kwargs: Any) -> None:
...
async def alarm_created(ctx: dict, alarm_id: str) -> None:
    ...
    async with ctx["sessionmaker"]() as session:
        alarm = await load_active_alarm(
            session, uuid.UUID(alarm_id), log=logger, log_extra={"alarm_id": alarm_id}
        )
        if alarm is None:
            return
        schedule = await notification.get_escalation_schedule(session)
        await enqueue_escalations(ctx["redis"], schedule, alarm_id=alarm_id, log=logger)
        enriched = await enrich_alarm_context(session, alarm)
        settings = ctx["settings"]
        ack_url = ack_url_for_alarm(alarm, settings, alarm_id=alarm_id, log=logger)
        await restore_zammad_ticket_id(session, alarm, successful_notification)
...
async def escalate(ctx: dict, alarm_id: str, step_no: int) -> None:
    ...
    async with ctx["sessionmaker"]() as session:
        alarm = await load_active_alarm(session, uuid.UUID(alarm_id), log=logger, log_extra=extra)
        if alarm is None:
            return
        ...
        ack_url = ack_url_for_alarm(alarm, settings, alarm_id=alarm_id, log=logger, log_extra=extra)
...
async def alarm_acked(
    ...
    async with ctx["sessionmaker"]() as session:
        ...
        delivery_error = ack_note_delivery_error(
            success, alarm_id=alarm_id, ticket_id=alarm.zammad_ticket_id, log=logger
        )
...
def _state_webhook_operations() -> StateWebhookOperations:
    """Build the explicit state-webhook dependency seam used by workers and tests."""
    return StateWebhookOperations(
        successful=successful_notification,
        url_allowed=partial(
            webhook_url_allowed,
            log_notification=log_notification,
            validate_url=validate_url_not_internal,
        ),
        build_payload=build_webhook_payload,
        headers_for_payload=webhook_headers,
        send=partial(send_state_webhook, log_notification=log_notification),
        log=logger,
    )
    ...
    async with ctx["sessionmaker"]() as session:
        ...
        try:
            await deliver_state_webhook(
                ...
                settings=settings,
                http=ctx["http"],
                operations=_state_webhook_operations(),
            )
