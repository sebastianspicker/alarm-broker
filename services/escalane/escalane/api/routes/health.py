...
from escalane.api.deps import get_app_settings, get_redis, get_sessionmaker, require_admin
from escalane.core.metrics import render_prometheus_metrics
from escalane.settings import Settings

router = APIRouter()

# Keep this value synchronized with the single Alembic head packaged in
# services/escalane/alembic/versions. The regression test verifies it.
...
# Export bounded aggregate metrics behind the same admin-key boundary as the API.
@router.get("/metrics", dependencies=[Depends(require_admin)])
async def metrics(
    sessionmaker: async_sessionmaker[AsyncSession] = Depends(get_sessionmaker),
) -> PlainTextResponse:
    from escalane.services.metrics_queries import get_alarm_counts, get_notification_counts
