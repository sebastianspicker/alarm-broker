...
def _create_master_data_tables() -> None:
    """Create the site, room, person, and device records used for enrichment."""
    op.create_table(
        "sites",
    ...
    op.create_table(
        ...
        sa.Column(
            "person_id", sa.String(), sa.ForeignKey("persons.id"), nullable=True, index=False
        ),
...
def _create_escalation_tables() -> None:
    """Create normalized escalation policy, target, and schedule tables."""
    op.create_table(
        "escalation_targets",
        sa.Column("id", sa.String(), primary_key=True),
        sa.Column("label", sa.String(), nullable=False),
...
def _create_alarm_tables() -> None:
    """Create the alarm lifecycle table and its notification history."""
    op.create_table(
        "alarms",
...
def _create_alarm_notification_tables() -> None:
    """Create durable per-channel notification attempt records."""
    op.create_table(
        "alarm_notifications",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column(
...
def upgrade() -> None:
    """Create the complete initial relational schema in dependency order."""
