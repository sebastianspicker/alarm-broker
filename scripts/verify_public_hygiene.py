...
FORBIDDEN_ARTIFACT_NAME_PATTERNS = (
    "*-ledger.md",
    "*_ledger.md",
    "agent-context*",
    "agent-memory*",
...
AGENT_TEXT_MARKERS = (
    b"you are an " + b"ai coding " + b"agent",
    b"instructions for " + b"codex",
    b"instructions for " + b"claude",
    b"instructions for " + b"copilot",
    b"agent " + b"handoff",
    b"use " + b"subagents",
    b"maintain this " + b"ledger",
    b"update memory " + b"after",
    b"generated by " + b"chatgpt",
    b"generated by " + b"claude",
    b"generated by " + b"codex",
    b"the agent " + b"implemented",
    b"the agent " + b"decided",
)
...
def _basename_reason(basename: str) -> str | None:
    ...
    if any(
        fnmatch.fnmatch(basename.lower(), pattern) for pattern in FORBIDDEN_ARTIFACT_NAME_PATTERNS
    ):
        return "development-assistant or process artifact filename"
    return None
...
def _text_content_reason(data: bytes, relative_path: str) -> str | None:
    ...
    if any(marker in data.lower() for marker in AGENT_TEXT_MARKERS):
        return "development-assistant instruction or provenance text"
...
def main(arguments: list[str] | None = None) -> int:
    ...
    print(f"Public repository hygiene check passed ({len(candidates)} candidate files).")
    return 0
