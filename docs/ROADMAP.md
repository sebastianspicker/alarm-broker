...
6. IP allowlist hardening — `YELK_IP_ALLOWLIST` defaults to blank (all IPs accepted). Add a startup `UserWarning` in `Settings` when the allowlist is empty and `simulation_enabled` is `False`.
7. Trigger response normalisation — unknown-token (404) vs. incomplete-mapping (409) leaks token validity to callers. Normalise both to a single 404 to prevent token-probing.
